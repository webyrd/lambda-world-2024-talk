#|
Will's experiments, 3--5 Oct 2024

* Removed `call-rel` tag for applications.
* Added implicit `delay` to `letrec-rel`
* Removed `conj` and `disj`
* Added `conj*`
* Added `conde`
* Generalized `fresh` to handle 0 or more variables and multiple goal expressions
* Generalized `run`^n/`run*` to handle multiple goal expressions
* Supplement `letrec-rel` with a `letrec-func` 
* Added more extensive Scheme evaluation.
* Updated and added tests.

TODO:

Add `=/=`, `symbolo`, `numbero`, `absento`
Add a parser with error messages, to help with this restricted language

Defer for now: add occurs-check (my first attempt caused some tests to fail--why?)


NOTE:

The grammar below is no longer accurate.
|#
...
(define (eval-gexpro expr s/c env $)
  (conde
    ...
    ;; WEB -- make `fresh` handle multiple variables and multiple goal
    ;; expressions
    ...
    [(fresh (id args params geb env1 ext-env vargs)
       ;; Call to relation
       ;; WEB formerly, tagged with `call-rel`
       (== `(,id . ,args) expr)
